...
def calcular_ano_ultrapassagem_populacional(
    ...
    if taxa_crescimento_populacao_menor > taxa_crescimento_populacao_maior:
        print(f"'A taxa de crescimento do país B ({taxa_crescimento_populacao_menor:.1f}%) deve ser menor do que a do país A ({taxa_crescimento_populacao_maior:.1f}%)'")
    ...
    while populacao_menor < populacao_maior:
        ...
        anos += 1
        print(f"'População de A, depois de {anos} ano(s) será de {int(populacao_menor)} pessoas, superando a de B, que será de {int(populacao_maior)} pessoas'")